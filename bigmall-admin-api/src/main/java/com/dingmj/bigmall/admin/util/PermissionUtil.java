package com.dingmj.bigmall.admin.util;
    ...
    public static List<PermVo> listPermVo(List<Permission> permissions){
        ...
        for (Permission permission : permissions){
            ...
            if (leftPerm == null){
                ...
                System.out.println("RequiresPermissions注解 VALUE:" + requiresPermissions.value()[0]);

                leftPerm.setLabel(button);
                leftPerm.setApi(api);

                System.out.println("API 是：" + api );

                perm2.getChildren().add(leftPerm);
            }else {
                throw new RuntimeException("权限已经存在，不能添加相同权限");
            }
